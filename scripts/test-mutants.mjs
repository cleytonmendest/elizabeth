...
const MUTANTES = [
  // ── A #118: cor nomeada não parece cor, e por isso ninguém a via ────────
  {
    ...
    arquivo: 'scripts/lint/rules/svgcolors.mjs',
    de: "/\\b(fill|stroke)=\"([^\"]*)\"/g",
    para: "/\\b(fill)=\"([^\"]*)\"/g",
    teste: 'tests/svgcolors.test.mjs',
  },
  {
    porque: '`currentColor` passa a ser acusado — a própria correção vira violação',
    arquivo: 'scripts/lint/rules/svgcolors.mjs',
    de: "const LEGITIMOS = new Set(['none', 'currentcolor', 'inherit', 'transparent']);",
    para: "const LEGITIMOS = new Set(['none']);",
    teste: 'tests/svgcolors.test.mjs',
  },
  {
    porque: 'a exceção registrada deixa de valer, e logo de bandeira vira violação',
    arquivo: 'scripts/lint/rules/svgcolors.mjs',
